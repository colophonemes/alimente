...
const debug = require('debug')('post-category-pagination')  // DEBUG=post-category-pagination
const paths = require('../../helpers/file-paths') // helper to get build system paths

/**
 * Post Category Pagination (Metalsmith plugin)
 *
 * Paginates Posts into their respective Categories
 *
 ...
 *
 */
function postCategoryPaginationPlugin (opts) {
  const defaults = {
  ...
  return function postCategoryPagination (files, metalsmith, done) {
...
module.exports = postCategoryPaginationPlugin
// require this plugin in ./tasks/metalsmith using:
// const postCategoryPagination = require(paths.lib('metalsmith/plugins/post-category-pagination.js'))
