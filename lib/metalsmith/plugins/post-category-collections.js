 ...
 *
 */
function postCategoryCollectionsPlugin (collectionsPlugin, paginationPlugin) {
  ...
  return function postCategoryCollections (files, metalsmith, done) {
    ...
    collectionsPlugin(collections)(files, metalsmith, () => {
      // then call the pagination plugin
      paginationPlugin(pagination)(files, metalsmith, done)
    })
