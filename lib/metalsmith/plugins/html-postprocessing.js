    ...
    Object.keys(files).filter(filter).forEach((file) => {
      ...
      $('img').each(function () {
        ...
        if (options.imageProtocol) {
          debug('Adding protocol to image')
          debug('Adding protocol to image')
        ...
        if (options.imageFigures) {
          const parent = img.parent()
          debug('Image wrapped in a <%s> tag', parent.name)
          if (parent[0] && parent[0].name === 'p') {
            debug('Wrapping image in a figure')
        ...
        if (options.imageResponsive) {
          debug('Setting .img-responsive class on image')
